...
- 관리자 이메일 로그인, 복수 내전 생성 및 실시간 신청 명단 동기화
- 신청 탭과 관리자 탭 분리
...
1. 관리자 탭에서 이메일 로그인 링크를 요청합니다.
2. 내전 이름과 신청 주소 코드를 입력해 내전을 생성합니다.
3. 생성된 참가 신청 링크를 참가자에게 공유합니다.
4. 참가자는 신청 탭에서 닉네임과 역할군 우선순위를 제출합니다. 랭크 조회는 선택입니다.
5. 관리자는 내전을 전환해 각각의 명단과 진행 데이터를 독립적으로 관리합니다.
